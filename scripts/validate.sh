#!/usr/bin/env bash
# Validação do EITA Reels Studio. Itens de MCP (Metricool, Kairogen)
...
echo "== 3. ElevenLabs =="
if grep -q '^ELEVENLABS_API_KEY=sk_' "$VIDEO_USE/.env" 2>/dev/null || [ -n "${ELEVENLABS_API_KEY:-}" ]; then
  echo "OK (chave presente; transcrição real gasta créditos, rodar sob demanda)"
else
  echo "PENDENTE: ELEVENLABS_API_KEY ausente no .env do video-use e no ambiente"
fi
...
echo " - Metricool: getBrandSettings lista a marca anaclaudia.eita com blog_id 6707687"
echo " - Kairogen: get_me_context mostra plano Essential+ e créditos (vídeo exige Essential)"
