#!/usr/bin/env bash
# Setup do EITA Reels Studio (Linux/cloud). No Mac, siga SETUP.md manualmente.
set -euo pipefail
...
echo "== 4/5 Python (PIL para overlays) =="
...
if [ ! -f "$VIDEO_USE/.env" ]; then
  echo "PENDENTE: gravar ELEVENLABS_API_KEY em $VIDEO_USE/.env (peça ao usuário)"
fi
